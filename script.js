...
let energy = 100;
const maxEnergy = 100;
const SERVER = 'http://37.53.92.226:3000'; // твой сервер или ngrok

function safeNumber(val, def=0){ return (val===undefined||val===null)?def:val; }
...
function updateBR(){
  playerData.BR=safeNumber(playerData.BR,100);
...
function login(){
  ...
  .then(data=>{
    if(data.error) alert(data.error);
      ...
      playerData={...data,
        BR:safeNumber(data.BR,100),
        balance:safeNumber(data.balance,0),
        level:safeNumber(data.level,1),
        lands:data.lands?JSON.parse(data.lands):lands,
        referrals:data.referrals?JSON.parse(data.referrals):[]
      };
...
function showContent(type){
  if(!playerData){ alert('Сначала войдите в игру!'); return; }
  ...
  if(type==='palace'){
    ...
    table.innerHTML=`<thead><tr><th>№</th><th>Имя</th><th>BR</th></tr></thead><tbody>Загрузка...</tbody>`;
    contentBox.appendChild(table);

    // --- Запрос топ игроков с сервера ---
    fetch(`${SERVER}/api/top`).then(res=>res.json()).then(users=>{
      users.sort((a,b)=>b.BR-a.BR);
      const tbody=users.map((u,i)=>`<tr><td>${i+1}</td><td>${u.username}</td><td>${safeNumber(u.BR,0).toFixed(1)}</td></tr>`).join('');
      table.querySelector('tbody').innerHTML=tbody;
    }).catch(err=>{ console.error(err); table.querySelector('tbody').innerHTML='<tr><td colspan="3">Ошибка загрузки рейтинга</td></tr>'; });
  }
  else if(type==='shop'){
