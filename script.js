...
async function register() {
  ...
  try {
    ...
    else { alert('Регистрация успешна! Войдите.'); backToChoice(); }
  } catch(e) {
    alert('Ошибка соединения с сервером: '+e.message);
  }
}
...
async function savePlayerData() {
  ...
  try {
    const res = await fetch(`${SERVER}/api/save/${playerData.id}`, {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
...
async function showContent(type) {
  ...
  if(type === 'palace') {
    ...
    try {
      const res = await fetch(`${SERVER}/api/rating`);
      if(!res.ok) throw new Error(`Сервер вернул ${res.status}`);
      ...
      mainText.innerText = `Ошибка загрузки рейтинга: ${e.message}`;
    }
  }
  else if(type === 'shop') {
    ...
    for(let i=0;i<9;i++){
      ...
      const nextCost = 5 * lands.filter(x=>x).length;
...
function updateReferralList() {
  ...
  if(!div) return;
  const refs = playerData.referrals||[];
  div.innerHTML = refs.length ? '<ul>'+refs.map(r=>`<li>${r}</li>`).join('')+'</ul>' : 'Пока нет рефералов';
}

// ===== Balance =====
function withdraw() { alert('Вывод (демо)'); }
function deposit() { playerData.balance=(playerData.balance||0)+5; alert('+5 TON'); showContent('balance'); }
...
function buyLand(index) {
  ...
  const cost = 5 * lands.filter(x=>x).length;
  if(playerData.balance<cost){ alert('Недостаточно TON'); return; }
  playerData.balance -= cost;
  lands[index] = true;
  savePlayerData();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function updateEnergy() { energyDisplay.innerText=`⚡ Энергия: ${energy}`; }

  setInterval(()=>{ if(energy<maxEnergy){ energy++; updateEnergy(); } }, 5000);
