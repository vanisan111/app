...
let energy = 100;
const maxEnergy = 100;
const SERVER = 'https://d72b5594823a.ngrok-free.app'; // замените на ваш внешний адрес или IP
...
function updateBR(){
  if(playerData && playerData.BR!=null)
...
function showContent(type){
  ...
  if(type==='palace'){
    ...
    fetch(`${SERVER}/api/top`)
      .then(res=>res.json())
      .then(rows=>{
        mainText.innerHTML=`🏆 <b>Рейтинг</b><br><br>`;
        const table=document.createElement('table');
        table.innerHTML=`<thead><tr><th>№</th><th>Имя</th><th>BR</th></tr></thead>
          <tbody>${rows.map((r,i)=>`<tr><td>${i+1}</td><td>${r.username}</td><td>${r.BR.toFixed(1)}</td></tr>`).join('')}</tbody>`;
